...
#define SENS_A A6
#define SENS_B A7
#define MAXIMUM_MILLIAMPRE 200  //for now
...
hall_sensor_data Hall_Data_Queue[2];

gyro_sensor_data Gyro_Data_Queue[2];

control_data Mag_Data_Queue[2];

control_data C2_Data_Queue[2];
...
void Task_Mag() {
  ...
  while (1) {
    Serial.println("in mag");
    //loop magnetorquer
    k_receive(Queue_Mag, &tmp_cmd, 0, &missed);  //if command receved
...
void setup() {
  ...
  Queue_Hall = k_crt_send_Q(2, sizeof(hall_sensor_data), Hall_Data_Queue);  // 4 elements of size
  Queue_Gyro = k_crt_send_Q(2, sizeof(gyro_sensor_data), Gyro_Data_Queue);  // 4 elements of size
  Queue_Mag = k_crt_send_Q(2, sizeof(control_data), Mag_Data_Queue);        // 4 elements of size
  Queue_C2 = k_crt_send_Q(2, sizeof(control_data), C2_Data_Queue);          // 4 elements of size
  ...
  Magnetic_Sem = k_crt_sem(1, 1);  // 0: start value, 1: max value (clipping)

  Init_SPI(SPI_MOSI, SPI_MISO, SPI_CLOCK);

  Serial.println(F("k_start"));

  k_start(); /* start krnl timer speed 1 milliseconds*/

  Serial.println(F("If you see this then krnl crashed :-("));
}
